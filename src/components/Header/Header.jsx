...
const Header = () => {
  ...
  return (
    <header className="bg-purple-700 text-white">
      <div className="px-4 py-2 flex justify-between w-screen items-center">
        ...
        {/* Navigation Links for Desktop */}
        <nav className="hidden md:flex gap-2 bg-transparent bg-opacity-95 w-full py-2 px-4 transition-all duration-300 ease-in-out">
          {navLinks.map((link) => (
            <Link
              key={link.to}
              to={link.to}
              className="block font-bold cursor-pointer p-2 border border-transparent rounded-md text-[#D3CCCC] hover:text-[#ff5e5e] hover:border-[#ff5e5e] transform transition-all duration-300 ease-in-out"
            >
              {link.label}
            </Link>
          ))}
